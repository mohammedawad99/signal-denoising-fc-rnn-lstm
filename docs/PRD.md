...
## 2. Problem statement
Given:
- a one-hot vector `C` of length `K = 4` indicating which of the four chosen frequencies generated the sample,
- a scalar `sigma` representing the noise standard deviation as a percentage of amplitude `A`,
- a context window of `T = 10` consecutive noisy samples `x_noisy ∈ R^T`,

predict the corresponding `T = 10` clean samples `y ∈ R^T`. The loss function is mean squared error (MSE) between prediction and ground truth.
...
The assignment statement defines the dataset entry but leaves the model interface implicit. We adopt the following interpretation, justified in §7:

> Each model receives `(x_noisy, C, sigma)` and predicts `y_clean` of the same length as `x_noisy`. Training minimizes `MSE(y_pred, y_clean)`.

For RNN/LSTM the noisy window is fed sequentially (one sample per timestep); `C` and `sigma` are broadcast and concatenated to every timestep, so the recurrent layer sees `[x_t, C, sigma]` of dimension `1 + K + 1 = 6` at each step. For FC, the entire window is flattened together with `C` and `sigma` into a vector of dimension `T + K + 1 = 15`.

## 7. Key assumptions (and why)
The assignment is partially open. For each open choice we record the choice and the reason.

- **A1. Four frequencies = `{1, 2, 5, 10}` Hz.** These frequencies cover low, medium, and relatively high frequencies while staying safely below the Nyquist limit for `fs = 50 Hz`. This range makes the comparison more meaningful because the models must handle both slowly and rapidly changing signals.
...
- **A6. Signal family = sine only (cosine is just a phase shift).** We add a random initial phase per signal in `[0, 2π)` so the network can't memorise a fixed waveform; this also covers the cosine case.
...
- **A8. Multiple signal realisations per `(frequency, sigma)` pair.** Each pair generates `N_realisations` independent signals (different phase, different noise draw) so the dataset is large enough to train a small network without overfitting trivially. Exact `N_realisations` chosen in `PRD_dataset.md`.
- **A9. Train/val/test split = 70/15/15 by realisation, stratified by `(frequency, sigma)`.** Splitting by realisation (not by window) prevents leakage where two windows from the same signal end up in different splits.
...
- **Model input:** `(x_noisy, C, sigma)` with `x_noisy ∈ R^10`, `C ∈ {0,1}^4` (exactly one 1), `sigma ∈ R`.
- **Model output:** `y_pred ∈ R^10`.
- **Training target:** `y_clean ∈ R^10`.
- **Loss:** `MSE(y_pred, y_clean)`.

## 9. Success criteria
- All three models train to convergence (validation loss plateaus) within the chosen budget.
- The comparison does not assume a fixed ranking between FC, RNN, and LSTM. Since the context window is short (`T = 10`), the FC model may perform competitively. The report will present the measured results honestly and discuss why each architecture performs as it does.
